...
function setupEventListeners() {
    ...
    document.getElementById('save-btn').addEventListener('click', handleJsonExport);
...
async function handlePurge() {
    ...
    if (txsToPurge.length > 0 && confirm(`This will permanently delete ${txsToPurge.length} reconciled transactions from the current account. Continue?`)) {
...
function handleJsonImport(e) {
    ...
    reader.onload = async (event) => {
        try {
            ...
            // Re-inserting without IDs lets IndexedDB generate new ones
    ...
    reader.onload = async (event) => {
        try {
            ...
            for(const line of lines){
                ...
                const tx = { accountId: currentAccountId };
                if(format === 'format1'){
            ...
            if (newTxs.length > 0 && confirm(`Found ${newTxs.length} transactions to import. Proceed?`)) {
                for (const tx of newTxs) await db.dbAdd('transactions', tx);
                alert(`${newTxs.length} transactions successfully imported.`);
                await loadTransactionsForCurrentAccount();
